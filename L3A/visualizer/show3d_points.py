...
def showpoints(xyz, c_gt=None, c_pred=None, waittime=0, showrot=False, magnifyBlue=0, freezerot=False,
    ...
    def render():
        ...
        h = show.shape[0]
        w = show.shape[1]
        depth = np.zeros((w, h), dtype='int32') - 10000
        # pattern = np.zeros((1, 6), dtype='float32')
        # for dx in range(-r, r+1):
        #     for dy in range(-r, r+1):
        #         if(dx**2+dy**2<r**2):
        #             dz = np.sqrt(float(r**2-dx**2-dy**2))
        #             pinfo = np.array([dx, dy, dz, dz/r, dz/r, dz/r], dtype='float32')
        #             if (pattern == np.zeros((1, 6), dtype='float32')).all():
        #                 pattern = pinfo
        #             else:
        #                 np.append(pattern, pinfo, axis=0)
        for i in range(ixyz.shape[0]):
            x = ixyz[i, 0]
            y = ixyz[i, 1]
            z = ixyz[i, 2]
            if x>0 and x<h and y>0 and y<w and depth[x, y]<z:
                depth[x, y] = z
                show[x, y, 0] = c2[i]# + z * 0.5
                show[x, y, 1] = c1[i]# + 30
                show[x, y, 2] = c0[i]# + z
...
if __name__ == '__main__':
    ...
    normal_channel = True
    data_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'points')
    data_list = os.listdir(data_dir)
    data_num = len(data_list)

    for i in range(data_num):
        point_set = np.loadtxt(os.path.join(data_dir, data_list[i]), delimiter=',').astype(np.float32)
